...
function RouteComponent() {
  ...
  const {
    ...
    const totalRkap =
      salesRealizationsList.reduce(
        (acc: number, item: any) =>
          acc + (item.rkapMonthly || 0),
        0
      );

      const exportToExcel = () => {
  const formattedData =
    salesRealizationsList.map(
      (item: any) => ({
        ProductBrand:
          item.productBrandName,
  ...
  <div className="container mx-auto space-y-8 px-4 py-8">
    ...
    <Card>
      <CardHeader className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <CardTitle>
            Sales Realization List
          </CardTitle>

          <CardDescription>
            Manage sales realization data
          </CardDescription>
        </div>
      ...
      <CardContent>
        ...
        <Button
          variant="outline"
          disabled={
            salesRealizationsList.length < 10
          }
          onClick={() =>
            setPage((prev) => prev + 1)
          }
