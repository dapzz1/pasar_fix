...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(
    orpc.admin.sale.daily_sales.delete.mutationOptions({
      ...
      onError: () => {
        toast.error('Failed to delete daily sales');
      },
    })
  );
  ...
  const exportToExcel = async () => {
    ...
    saveAs(fileData, 'daily-sales.xlsx');
  };

  return (
    <div className="container mx-auto space-y-8 px-4 py-8">
      <div className="text-left">
        <h1 className="font-bold text-3xl">Manage Daily Sales</h1>

        <p className="text-slate-600">Manage daily sales data</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card>
          <CardContent className="p-6">
            ...
            <h2 className="mt-2 font-bold text-3xl">{dailySalesList.length}</h2>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            <p className="text-muted-foreground text-sm">Total Qty</p>
      ...
      <Card>
        <CardHeader className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <CardDescription>Manage daily sales data</CardDescription>
          </div>
          ...
          {!isLoading && dailySalesList.length > 0 && (
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {dailySalesList.map((item) => (
                    <tr className="border-b hover:bg-muted/30" key={item.id}>
                      ...
                      <td className="p-3 text-right">{item.revenue}</td>
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <Button
                  disabled={dailySalesList.length < limit}
                  onClick={() => setPage((prev) => prev + 1)}
                  variant="outline"
                >
                  Next
